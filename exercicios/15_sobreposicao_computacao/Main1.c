...
int main(int argc, char *argv[])
{
    ...
    int total_inner = N - 2; // Células internas (não bordas globais)
    int nlocal = total_inner / size;
    int rest = total_inner % size;
    int local_cells = nlocal + (rank < rest ? 1 : 0);

    // Tamanho do vetor local incluindo células fantasmas
    ...
    {
        // Trocar bordas com vizinhos usando Send/Recv
        // Enviar borda direita e receber da esquerda
        if (right_rank != MPI_PROC_NULL)
        {
            MPI_Send(&u_old[local_cells], 1, MPI_DOUBLE, right_rank, 0, MPI_COMM_WORLD);
            MPI_Recv(&u_old[local_cells + 1], 1, MPI_DOUBLE, right_rank, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }

        // Enviar borda esquerda e receber da direita
        if (left_rank != MPI_PROC_NULL)
        {
            MPI_Send(&u_old[1], 1, MPI_DOUBLE, left_rank, 1, MPI_COMM_WORLD);
            MPI_Recv(&u_old[0], 1, MPI_DOUBLE, left_rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
    ...
    if (rank == 0)
    {
        printf("Tempo da versão 1 (Send/Recv): %.6f segundos\n", max_time);
    }
